...
void pinMode(uint8_t pin, uint8_t mode)
{
    ...
    if ((modeVal & 0xf0) != 0) {
        WR32(GPPUD, modeVal >> 4);
        lowlev_cycleDelay(150);
        if (pin < 32)
            WR32(GPPUDCLK0, 1 << pin);
        else
            WR32(GPPUDCLK1, 1 << (pin - 32));
        lowlev_cycleDelay(150);
        WR32(GPPUD, 0);
        if (pin < 32)
            WR32(GPPUDCLK0, 0);
...
void digitalWrite(uint8_t pin, int val)
{
    if (val) {
        if (pin < 32)
            ...
            WR32(GPSET1, 1 << (pin - 32));
    } else {
        if (pin < 32)
            WR32(GPCLR0, 1 << pin);
        else
            WR32(GPCLR1, 1 << (pin - 32));
    }
}
...
{
    if (pin < 32)
        return ((RD32(GPLEV0) >> pin) & 0x01) != 0;
    return ((RD32(GPLEV1) >> (pin - 32)) & 0x01) != 0;
}