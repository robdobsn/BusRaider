...
void state_fun_final_letter(char ch, scn_state* state)
{
    ...
    switch (ch) {
    case 'l':
        if (state->private_mode_char == '?' && state->cmd_params_size == 1 && state->cmd_params[0] == 25) {
    ...
    case 'm':
        ...
        if (state->cmd_params_size == 3 && state->cmd_params[0] == 38 && state->cmd_params[1] == 5) {
            gfx_set_fg(state->cmd_params[2]);
            goto back_to_normal;
        }
        if (state->cmd_params_size == 3 && state->cmd_params[0] == 48 && state->cmd_params[1] == 5) {
