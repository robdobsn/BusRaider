...
static const char* MODULE_PREFIX = "SpectrumZ80Format";
...
McZXSpectrumZ80Format::McZXSpectrumZ80Format()
{
}

int McZXSpectrumZ80Format::decompress(uint8_t* pIn, uint8_t* pDest, int maxLen, bool isCompressed)
{
    uint8_t* pOut = pDest;
...
bool McZXSpectrumZ80Format::proc([[maybe_unused]] FileParserDataCallback* pDataCallback, 
            ...
            [[maybe_unused]] const uint8_t* pData, [[maybe_unused]] int dataLen)
{
    ...
    if (versionNum == 1)
    {
        // LogWrite(MODULE_PREFIX, LOG_DEBUG, "Version 1 Z80 file headerDataStartPos %d, compressed %c", ramDumpOffset, isCompressed ? 'Y' : 'N');
        totalLenDecoded = decompress(sna->extraData + ramDumpOffset, ram, 49152, isCompressed); 
        pDataCallback(16384, ram, 49152, pCallbackParam);
    }
    else
    {
        ...
        while (true)
        {
            ...
            // LogWrite(MODULE_PREFIX, LOG_DEBUG, "Decoding block curPos %d blockLen %d page %d", curPos, compressedBlockLen, blockPage);
            switch(blockPage)
            {
            ...
            if (blockBase != -1)
            {
                ...
                if (lenDecoded != 16384)
                    LogWrite(MODULE_PREFIX, LOG_DEBUG, "Decode block len %d != 16384", lenDecoded);
                totalLenDecoded += lenDecoded;
                pDataCallback(blockBase, ram, 16384, pCallbackParam);
            }
    ...
    // Done
    // LogWrite(MODULE_PREFIX, LOG_DEBUG, "OK format (v%d hdrLen %d compr %s) input len %d -> len %d run from %04x",
