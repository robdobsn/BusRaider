...
void te_free_parameters(te_expr *n) {
	...
	case TE_FUNCTION7: case TE_CLOSURE7: te_free(n->parameters[6]); __attribute__((fallthrough));
	case TE_FUNCTION6: case TE_CLOSURE6: te_free(n->parameters[5]); __attribute__((fallthrough));
	case TE_FUNCTION5: case TE_CLOSURE5: te_free(n->parameters[4]); __attribute__((fallthrough));
...
static double randomv() { return rand() / ((double)RAND_MAX); };
...
static const te_variable functions[] = {
	...
	{ "atanh", atanh,  TE_FUNCTION1 | TE_FLAG_PURE, 0 },
			...
			if ((s->next[0] >= 'a' && s->next[0] <= 'z') || (s->next[0] >= 'A' && s->next[0] <= 'Z') || (s->next[0] == '_') || (s->next[0] == '$')) {
				...
				if (!var) {
					...
					case TE_CLOSURE4: case TE_CLOSURE5: case TE_CLOSURE6: case TE_CLOSURE7:
						s->context = var->context; __attribute__((fallthrough));
