...
private:
    ...
    public:
        ...
        bool extract(const uint8_t* pBuf, uint32_t bufLen)
        {
            // First two bytes
            uint32_t pos = 0;
            if (bufLen < pos + 2)
    ...
    // Helpers
    void handleRxPacketData(const uint8_t* pBuf, uint32_t bufLen);
    bool extractWSHeaderInfo(const uint8_t* pBuf, uint32_t bufLen);
    void unmaskData();
