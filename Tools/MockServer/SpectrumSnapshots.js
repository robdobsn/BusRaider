// This code is from jsspeccy3 by gasman
...
function parseZ80File(data) {
    ...
    if (snapshot.registers.PC !== 0) {
        ...
        snapshot.memoryPages[1] = new Uint8Array(memory.buffer, 0x4000, 0x4000);
        snapshot.memoryPages[2] = new Uint8Array(memory.buffer, 0x8000, 0x4000);
        console.log(`Z80 V1 ${memory.length} bytes`);
        ...
        if (is48K) {
            ...
            console.log(`Z80 V2/3 48K ${additionalHeaderLength} bytes`);
        } else {
            pageIdToNumber = {
                3: 0,
                4: 1,
        ...
        while (offset < data.byteLength) {
            ...
            if (pageId in pageIdToNumber) {
                ...
                console.log(`Z80 V2/3 memPage ${pageId} => ${pageNumber} ${pageData.length} bytes`);
            }
