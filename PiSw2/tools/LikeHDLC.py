...
def calcCRC(data):
    crc = 0xffff
    for c in data:
        crc = ((crc<<8)&0xff00) ^ CRC16_LUT[((crc>>8)&0xff)^c]
...
class Frame(object):
    ...
    @classmethod
    def setEscapes(cls, asciiEscapes):
        if asciiEscapes:
            cls.FRAME_DELIMITER = 0x7E
            cls.ESCAPE_CODE = 0x7D
...
# LikeHDLC
class LikeHDLC:
    def __init__(self, onFrame, onError, asciiEscapes):
        self.currentFrame = None
        self.payloadsAreStrings = False
        self.onFrame = onFrame
        self.onError = onError
        Frame.setEscapes(asciiEscapes)
