...
------------------------------------------------------------------------------*/
void make_byte( int sw, char *buf, int num, int max_buf_len )
{
	switch( sw ) {
	case INTEL:
		snprintf( buf, max_buf_len, "0%02XH", num&0xff );
		break;
	case MOTOROLA:
		snprintf( buf, max_buf_len, "$%02X", num&0xff );
		break;
	case C_LANG:
		snprintf( buf, max_buf_len, "0x%02X", num&0xff );
		break;
	case JUSTHEX:
		snprintf( buf, max_buf_len, "%02X", num&0xff );
		break;
...
------------------------------------------------------------------------------*/
void make_ofs_byte( int sw, char *buf, int num, int max_buf_len )
{
	if( num >= 0x80 ) {
		*buf = '-';
		num = (num^0xff)+1;
...
------------------------------------------------------------------------------*/
void make_word( int sw, char *buf, int num, int max_buf_len )
{
	switch( sw ) {
	case INTEL:
		snprintf( buf, max_buf_len, "0%04XH", num&0xffff );
		break;
	case MOTOROLA:
		snprintf( buf, max_buf_len, "$%04X", num&0xffff );
		break;
	case C_LANG:
		snprintf( buf, max_buf_len, "0x%04X", num&0xffff );
		break;
	case JUSTHEX:
		snprintf( buf, max_buf_len, "%04X", num&0xffff );
		break;
...
------------------------------------------------------------------------------*/
int disasmZ80( uchar *buf, ulong base, ulong addr, char *stream, int sw, int nullcheck, int jsonFormat, uint32_t max_buf_len )
{
	disZ80data	*data;
	int		i, st;
	int		*op_type;
	...
	// output for address
	if (jsonFormat)
		st = snprintf(stream, max_buf_len, "%04X ", base+addr);
	else
		st = snprintf(stream, max_buf_len, "%04X :", base+addr);
	...
	if (jsonFormat)
	{
		for ( i = 0; i < data->len; i++ ) {
			st += snprintf( &stream[st], max_buf_len, "%02X", buf[addr+i] );
		}
		for ( i = 5; i > data->len; i--) {
			strlcat(&stream[st], "  ", max_buf_len);
			st += 2;
		}
		st += snprintf( &stream[st], max_buf_len, "%s~", nimonic[data->nimonic]);
		// // write nimonic
		// const char* nimonicStr = nimonic[data->nimonic];
		// for (unsigned int i = 0; i < strlen(nimonicStr); i++)
		// 	stream[st++] = rdtoupper(nimonicStr[i]);
		// stream[st++] = ' ';
	}
	else
	{
		...
		switch( data->len ) {
		case 4:
			st += snprintf( &stream[st], max_buf_len, "\t" );
			break;
		default:
			st += snprintf( &stream[st], max_buf_len, "\t\t" );
			break;
	...
	if( data->op_num != 0 ) {
		...
		for( i = 0; i < data->op_num; i++ ) {
			...
			if( *op_type < OT_IMM_BYTE ) {
				st += snprintf( &stream[st], max_buf_len, optype0[*op_type] );
				line += strlen( optype0[*op_type] );
				op_type++;
			} else {
				switch( *op_type ) {
				case OT_NUL:
					make_byte( sw, temp, buf[addr+ofs], max_buf_len );
					st += snprintf( &stream[st], max_buf_len, temp );
					line += strlen( temp );
					ofs++;
					break;
				case OT_IMM_BYTE:
				...
				case OT_IMM_PORT:
					make_byte( sw, temp, buf[addr+ofs], max_buf_len );
					st += snprintf( &stream[st], max_buf_len, "(%s)", temp );
					line += strlen( temp )+2;
					ofs++;
					op_type++;
					break;
				case OT_RST:
					make_byte( sw, temp, buf[addr]-0xc7, max_buf_len );
					st += snprintf( &stream[st], max_buf_len, temp );
					line += strlen( temp );
					op_type++;
					break;
				case OT_IDX_IX:
					make_ofs_byte( sw, temp, buf[addr+ofs], max_buf_len );
					st += snprintf( &stream[st], max_buf_len, "(ix%s)", temp );
					line += strlen( temp )+4;
					ofs++;
					op_type++;
					break;
				case OT_IDX_IY:
					make_ofs_byte( sw, temp, buf[addr+ofs], max_buf_len );
					st += snprintf( &stream[st], max_buf_len, "(iy%s)", temp );
					line += strlen( temp )+4;
					ofs++;
					op_type++;
					break;
				case OT_REL:
					...
					make_word( sw, temp, base+addr+rel+2, max_buf_len );
					st += snprintf( &stream[st], max_buf_len, temp );
					line += strlen( temp );
					ofs++;
					op_type++;
					break;
				case OT_IMM_WORD:
					make_word( sw, temp, (buf[addr+ofs+1]<<8)+buf[addr+ofs], max_buf_len );
					st += snprintf( &stream[st], max_buf_len, temp );
					line += strlen( temp );
					ofs += 2;
					op_type++;
