...
bool CPUHandler::debuggerHandleWaitCycle(uint32_t addr, uint32_t data, uint32_t rawBusVals)
{
    ...
    while (snippetIdx < sizeof(regQueryInstructions))
    {
...
#endif
            // Clear the wait
            _busRawAccess.waitResetFlipFlops();    
        }
        else
        {
            ...
            _busRawAccess.pibSetValue(injectVal & 0xff);
            ...
            if (memoryGrab)
            {
                ...
                if (busAckOk)
                {
                    ...
                    _busRawAccess.rawBlockRead(0, memContents, STD_TARGET_MEMORY_LEN, BLOCK_ACCESS_MEM);

                    // Send to mem controller
                    _memoryInterface.memCacheBlockWrite(0, memContents, STD_TARGET_MEMORY_LEN, BLOCK_ACCESS_MEM);
        ...
        _busRawAccess.addrAndDataBusRead(addr, data);
    }
