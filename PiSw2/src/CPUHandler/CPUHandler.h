...
typedef void CPUCycleCBType(void* pObject, uint32_t addr, uint32_t data, uint32_t flags, uint32_t& curRetVal);
typedef void CPUBusReqCBType(void* pObject, BR_BUS_REQ_REASON reason, BR_RETURN_TYPE rslt);
...
public:
    // Constructor
    CPUHandler(BusRawAccess& busRawAccess, 
    ...
    void registerCallbacks(CPUCycleCBType* pBusAccessCB, 
                CPUBusReqCBType* pBusReqAckedCB, 
                void* pObject)
    {
    ...
    {
        return _busRawAccess.rawBlockWrite(addr, pData, len, accessType);
    }
    BR_RETURN_TYPE blockRead(uint32_t addr, uint8_t* pData, uint32_t len, BlockAccessType accessType)
    {
        return _busRawAccess.rawBlockRead(addr, pData, len, accessType);
    }
...
private:
    ...
    CPUProgrammerIF& _cpuProgrammer;

    // CPU settings
    CPUSettings _cpuSettings;
    ...
    public:
        ...
        CPUBusReqCBType* _busReqAckedCB;
        void* _pObject;
        uint32_t _socketIdx;
    ...
    CPUCycleCBType* _pBusAccessCB;
    CPUBusReqCBType* _pBusReqAckedCB;
    void* _pBusCBObject;
