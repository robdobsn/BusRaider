		...
		if (GetMultipartFormPart (&pPartHeader, &pPartData, &nPartLength))
		{
			    ...
			    && nPartLength > 0)
			{
				...
				if (pKernelImage != 0)
				{
					...
					unsigned rxCRC = MiniHDLC::computeCRC16(pKernelImage, nPartLength);
					CLogger::Get ()->Write (FromHTTPBootServer, LogDebug,
								"OTA update length %u CRC %04x", nPartLength, rxCRC);
