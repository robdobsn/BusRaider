...
// Module name
static const char MODULE_PREFIX[] = "TargetBreakpoints";
...
void TargetBreakpoints::clear()
{
    for (int i = 0; i < MAX_BREAKPOINTS; i++)
    {
        ...
        snprintf(_fastBreakpoints[i].hitMessage, sizeof(_fastBreakpoints[i].hitMessage), "FB%02d", i);
    }
...
bool TargetBreakpoints::checkForBreak( uint32_t addr,  uint32_t data, 
        uint32_t flags,  uint32_t& retVal)
{
    // LogWrite(MODULE_PREFIX, LOG_DEBUG, "checkForBreak %04x, fast %d", 
...
void TargetBreakpoints::setFastBreakpoint(uint32_t addr, bool en)
{
    ...
    for (int i = 0; i < _fastBreakpointsNumEnabled; i++)
    {
        ...
        {
            // LogWrite(MODULE_PREFIX, LOG_DEBUG, "Fast breakpoint addr %04x now %d", addr, en);
